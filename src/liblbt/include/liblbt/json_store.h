...
public:
    ...
    // Writes to disk (dir/key.json), updates memory, suppresses self-watch echoes
    bool put(const std::string& key, const Json::Value& value);

    // Deletes file and removes from memory
...
private:
    ...
    // Track files we just wrote to ignore their inotify events for a short time
    ...
    void loadAllUnlocked(); // call with mtx_ held
    void watchLoop();
