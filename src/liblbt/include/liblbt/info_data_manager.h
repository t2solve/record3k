#pragma once
#include <liblbt/json_store.h>
#include <json/json.h>
#include <filesystem>
#include <string>
#include <unordered_map>
#include <memory>
#include <mutex>
#include <optional>

// Manages category-specific JsonStore instances for /info/* data.
// Categories (directory names): cameras, pipelines, calibrations, records, files, studies
class InfoDataManager {
public:
    explicit InfoDataManager(std::filesystem::path baseDir);
    void start();
    void stop();

    // Returns array of JSON objects for a category.
    Json::Value list(const std::string& category);
    // Returns single JSON object if exists.
    std::optional<Json::Value> get(const std::string& category, const std::string& key);

private:
    struct Cat { std::unique_ptr<JsonStore> store; };
    std::filesystem::path baseDir_;
    std::unordered_map<std::string, Cat> cats_;
    std::mutex mtx_;
    void ensureStartedUnlocked();
};

// Global accessor (singleton-style).
InfoDataManager& infoDataManager();
